...
// Helper untuk endpoint yang hanya boleh diakses wali kelas atau admin
// (mis. rekap & rapor, peringkat, template dokumen, pengaturan rapor).
// Mengembalikan {error, status, guru:null} jika ditolak, atau {guru} jika boleh lanjut.
async function requireWaliKelasOrAdmin(req) {
  const guru = await getGuruFromReq(req);
  if (!guru) return { status: 401, error: 'Anda belum masuk.' };
  if (guru.role !== 'admin' && guru.role !== 'wali_kelas') {
    return { status: 403, error: 'Hanya wali kelas atau admin yang boleh mengakses data ini.' };
  }
  return null;
}
